...
type withWriteLockProvidedMap[K comparable, V any] interface {
	Load(key K) (V, bool)
...
type lockableMapImpl[K comparable, V any] interface {
	Map[K, V]
	WithWriteLock(f func(Map[K, V]))
}

func NewLockableMap[K comparable, V any]() lockableMapImpl[K, V] {
	return &lockMap[K, V]{}
}
