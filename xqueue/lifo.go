...
type lifoOpts[T any] struct{}

func (lifoOpts[T]) InitialCapacity(n int) LIFOOption[T] {
...
func (lifoOpts[T]) MaxCapacity(n int) LIFOOption[T] {
	return func(cfg *lifoConfig[T]) {
		cfg.maxCapacity = n
...
func (lifoOpts[T]) CappedBuffer(p []T) LIFOOption[T] {
	return func(cfg *lifoConfig[T]) {
		cfg.cappedBuffer = p
...
// IntoIter returns an iterator that yields the elements in LIFO order.
//
// The iterator locks the underlying stack in read-mode during
// iteration to ensure consistency.
//
// Callers must not attempt to lock the queue for writing while
// an iterator is in use in the same goroutine. Failing to do so
